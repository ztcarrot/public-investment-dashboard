...
def main():
    ...
    if not has_custom_assets:
        ...
        with st.expander("📖 了解更多", expanded=False):
            ...
            这是一个**永久投资组合追踪工具**，专注于分析经典的**四分类资产配置**：

            💡 **什么是"永久投资组合"？**

            永久投资组合是一种经典的资产配置策略，将资产分配到**四个核心类别**：
            - **📈 股票**：追求增长，承担较高风险
            - **🏅 黄金**：保值避险，对抗通胀
            - **💰 现金**：流动性储备，应对急需
            - **🏛️ 国债**：稳定收益，降低风险
            ...
            这个应用帮助你：
            - 📊 **追踪资产配置**：实时查看四大类资产的分布和变化
            - 📈 **分析投资收益**：总资产走势、年化增长率、最大回撤等关键指标
            - 🔄 **对比历史表现**：回溯历史数据，分析不同时间段的收益
            - 🎨 **可视化展示**：多种图表直观展示投资组合变化
            - 📋 **评估贡献度**：了解每个资产类别对整体收益的贡献
            ...
            **第一步：配置您自己的资产**
            ...
            2. 添加您持有的资产：
               - 输入资产代码（如：511130）
               - 选择资产类型（场内ETF/基金/股票/债券）
               - 设置持有份额
            3. 点击 **💾 保存** 按钮
            4. 配置会自动保存到网址中，方便下次访问
            ...
            **第二步：分析您的投资**
            ...
            - ✅ **URL 配置存储**：配置保存在网址中，无需登录
            - ✅ **多图表展示**：走势图、饼图、堆叠图等
            - ✅ **智能分析**：最大回撤、年化增长率、恢复期等
            - ✅ **数据导出**：支持导出配置文件

            ### 📌 配置提示

            配置完成后，您的专属配置网址会自动生成，包含：
            - 您的资产列表
            - 您选择的开始日期

            **保存这个网址**，下次访问时会自动加载您的配置！
